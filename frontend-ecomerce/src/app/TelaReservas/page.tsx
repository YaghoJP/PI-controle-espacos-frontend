...
import SpaceReserva from "../Componets/SpaceReserva" // Importa o componente e seu tipo
import { Filter } from "lucide-react";
import { API_BASE_URL } from "../Service/localhost";
import { handlerDeleteReservation } from "../Service/service";
...
export default function DashboardPage() {
  ...
  const [spacesData, setSpacesData] = useState<Reservation[]>([]);
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {spacesData.slice(0, spacesData.length).map((space) => (
              < SpaceReserva key={space.id} space={space} onDelete={handlerDeleteReservation} />
            ))}
          </div>
        </div>
      </main>
